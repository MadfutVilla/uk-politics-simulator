...
/* v4.4 guaranteed Campaign HQ functionality */
window.campaignStateV44 = window.campaignStateV44 || {volunteers:0,offices:0,adverts:0,actionsThisWeek:0,maxActionsPerWeek:3,regions:[],seats:[],log:[]};

function ensureCampaignV44(){
  const s=window.campaignStateV44;
  if(s.regions.length) return;
  const base=fixedHq?.pollingHistory?.[fixedHq.pollingHistory.length-1]||generatedPolling?.party||5.2;
  const members=generatedMembership?.total||14310;
  s.volunteers=Math.max(900,Math.round(members*.18)); s.offices=1; s.adverts=0; s.actionsThisWeek=0; s.maxActionsPerWeek=3;
  s.regions=[
    {name:selectedHQ||"West Midlands",support:+(base+4.1).toFixed(1),office:true},
    {name:"London",support:+Math.max(1,base-.4).toFixed(1),office:false},
    {name:"North West",support:+Math.max(1,base-.1).toFixed(1),office:false},
    {name:"South East",support:+Math.max(1,base-.9).toFixed(1),office:false},
  ...
  s.seats=[
    {name:`${selectedHQ||"Birmingham"} Central`,party:"Labour",chance:14,target:false},
  ...
  s.log=["Campaign HQ opened.","Initial regional strategy prepared.","Target-seat list created."];
}

function campaignCanV44(cost){
  const s=window.campaignStateV44;
  if(s.actionsThisWeek>=s.maxActionsPerWeek){
    fixedHq.news.unshift(createFixedNews("Campaign HQ","No campaign actions left","You have used all campaign actions this week. End the week to continue.","major",[]));
    renderFixedHq(); openHqSection("campaign"); return false;
  }
  if(fixedHq.funds<cost){
    fixedHq.news.unshift(createFixedNews("Finance Office","Not enough funds","Your campaign cannot afford this action yet.","breaking",[]));
    renderFixedHq(); openHqSection("campaign"); return false;
  }
  return true;
}
function campaignSpendV44(cost,log){const s=window.campaignStateV44;fixedHq.funds-=cost;s.actionsThisWeek++;s.log.unshift(log);s.log=s.log.slice(0,8);}
function campaignBoostV44(amount){const i=fixedHq.pollingHistory.length-1;fixedHq.pollingHistory[i]=+(fixedHq.pollingHistory[i]+amount).toFixed(1);}

function runCampaignActionV44(type){
  ensureCampaignV44(); const s=window.campaignStateV44;
  if(type==="rally"){if(!campaignCanV44(75000))return;campaignSpendV44(75000,"Held a launch rally.");campaignBoostV44(.2);s.volunteers+=180;if(generatedMembership)generatedMembership.total+=450;fixedHq.news.unshift(createFixedNews("Campaign HQ","Rally energises supporters","Your campaign rally boosts membership, volunteers and visibility.","positive",[]));}
  if(type==="advert"){if(!campaignCanV44(250000))return;campaignSpendV44(250000,"Launched a national advert campaign.");campaignBoostV44(.35);s.adverts++;fixedHq.news.unshift(createFixedNews("Campaign HQ","Advertising campaign goes live","Your adverts increase visibility but reduce campaign funds.","major",[]));}
  if(type==="volunteers"){if(!campaignCanV44(40000))return;campaignSpendV44(40000,"Ran a volunteer recruitment drive.");s.volunteers+=650;if(generatedMembership)generatedMembership.total+=280;fixedHq.news.unshift(createFixedNews("Campaign HQ","Volunteer drive succeeds","New activists join the campaign operation.","positive",[]));}
  if(type==="target"){if(!campaignCanV44(95000))return;campaignSpendV44(95000,"Expanded target-seat campaign.");s.seats.forEach(seat=>{if(seat.target)seat.chance=Math.min(45,seat.chance+4)});campaignBoostV44(.08);fixedHq.news.unshift(createFixedNews("Campaign HQ","Target-seat operation expands","Your selected target seats become more competitive.","positive",[]));}
  if(type==="tour"){if(!campaignCanV44(120000))return;campaignSpendV44(120000,"Completed a regional tour.");s.regions.forEach(r=>{r.support=+(r.support+(r.office?.6:.25)).toFixed(1)});campaignBoostV44(.18);fixedHq.news.unshift(createFixedNews("Campaign HQ","Regional tour completed","The leader's regional tour improves campaign reach.","positive",[]));}
  renderFixedHq(); openHqSection("campaign");
}

function toggleTargetSeatV44(i){ensureCampaignV44();const seat=campaignStateV44.seats[i];if(!seat)return;seat.target=!seat.target;campaignStateV44.log.unshift(`${seat.target?"Targeted":"Removed"} ${seat.name}.`);openHqSection("campaign");}
function openCampaignOfficeV44(i){ensureCampaignV44();const s=campaignStateV44,r=s.regions[i];if(!r||r.office)return;if(!campaignCanV44(180000))return;r.office=true;r.support=+(r.support+1.2).toFixed(1);s.offices++;campaignSpendV44(180000,`Opened campaign office in ${r.name}.`);fixedHq.news.unshift(createFixedNews("Campaign HQ",`New office opens in ${r.name}`,"The office improves regional support and local organising.","positive",[]));renderFixedHq();openHqSection("campaign");}

function renderCampaignHqV44(){
  ensureCampaignV44(); const s=window.campaignStateV44;
  const polling=fixedHq?.pollingHistory?.[fixedHq.pollingHistory.length-1]||0;
  const actionsLeft=s.maxActionsPerWeek-s.actionsThisWeek;
  const selectedSeats=s.seats.filter(x=>x.target).length;
  const readiness=Math.min(100,Math.round(18+selectedSeats*8+s.offices*7+s.adverts*6+actionsLeft*2));
  return `
  <div class="campaign-v44-grid">
    <div class="campaign-v44-card"><h3>Campaign Overview</h3><div class="campaign-v44-stats">
      <div><span>Polling</span><strong>${polling.toFixed(1)}%</strong></div><div><span>Funds</span><strong>${fixedMoney(fixedHq.funds)}</strong></div>
      <div><span>Volunteers</span><strong>${s.volunteers.toLocaleString("en-GB")}</strong></div><div><span>Actions Left</span><strong>${actionsLeft}</strong></div>
    </div></div>
    <div class="campaign-v44-card"><h3>Campaign Actions</h3><div class="campaign-v44-list">
      ${[
      ...
      ].map(([a,b,c])=>`<div class="campaign-v44-row"><div><strong>${a}</strong><p>${b}</p></div><button onclick="runCampaignActionV44('${c}')" ${actionsLeft<=0?"disabled":""}>Run</button></div>`).join("")}
    </div></div>
    <div class="campaign-v44-card"><h3>Election Readiness</h3><div class="campaign-v44-stats">
      <div><span>Target Seats</span><strong>${selectedSeats}</strong></div><div><span>Offices</span><strong>${s.offices}</strong></div>
      <div><span>Adverts</span><strong>${s.adverts}</strong></div><div><span>Readiness</span><strong>${readiness}/100</strong></div>
    </div></div>
    <div class="campaign-v44-card"><h3>Regional Support</h3><div class="campaign-v44-list">
      ${s.regions.map((r,i)=>`<div class="campaign-v44-row"><div><strong>${r.name}</strong><small>${r.office?"Office active":"No office"} • Support ${r.support.toFixed(1)}%</small><div class="campaign-v44-meter"><div style="--w:${Math.min(100,r.support*5)}%;"></div></div></div><button onclick="openCampaignOfficeV44(${i})" ${r.office?"disabled":""}>${r.office?"Open":"Open Office"}</button></div>`).join("")}
    </div></div>
    <div class="campaign-v44-card"><h3>Target Seats</h3><div class="campaign-v44-list">
      ${s.seats.map((seat,i)=>`<div class="campaign-v44-row"><div><strong>${seat.name}</strong><small>Held by ${seat.party} • Win chance ${seat.chance}% ${seat.target?"• Targeted":""}</small><div class="campaign-v44-meter"><div style="--w:${Math.min(100,seat.chance*2)}%;"></div></div></div><button onclick="toggleTargetSeatV44(${i})">${seat.target?"Remove":"Target"}</button></div>`).join("")}
    </div></div>
    <div class="campaign-v44-card"><h3>Campaign Log</h3><div class="campaign-v44-list">
      ${s.log.map(log=>`<div class="campaign-v44-row"><div><strong>${log}</strong><small>Week ${fixedHq.week}</small></div></div>`).join("")}
    </div></div>
  </div>`;
}

openHqSection=function(section){
  if(section==="dashboard"){showDashboardHome();return;}
  const home=document.getElementById("fixedDashboardHome"), detail=document.getElementById("fixedDashboardDetail"), title=document.getElementById("hqDetailTitle"), content=document.getElementById("hqDetailContent");
  if(!home||!detail||!title||!content)return;
  home.classList.remove("force-visible");home.classList.add("force-hidden");home.style.display="none";
  detail.classList.remove("force-hidden");detail.classList.add("force-visible");detail.style.display="block";
  document.querySelectorAll(".fixed-hq-nav").forEach(btn=>{btn.classList.remove("active"); if(btn.textContent.toLowerCase().includes(section.toLowerCase())) btn.classList.add("active");});
  if(section==="campaign"){title.textContent="Campaign HQ";content.classList.add("campaign-v44-open");content.innerHTML=renderCampaignHqV44();return;}
  content.classList.remove("campaign-v44-open");
  const map={media:"Media Centre",policy:"Policy Unit",candidates:"Candidate Recruitment",party:"Party Management",finance:"Finance Office",research:"Research Department",map:"UK Campaign Map",polling:"Polling Centre",calendar:"Calendar"};
  title.textContent=map[section]||"Section";
  content.innerHTML=`<div class="fixed-detail-card"><h3>${title.textContent}</h3><p>This section is ready to be expanded next. For now, use Campaign HQ, Dashboard actions and End Week to progress the game.</p><button onclick="runQuickAction('policy')">Take Basic Action</button></div>`;
};

const prevEndWeekV44=endWeek;
endWeek=function(){prevEndWeekV44();ensureCampaignV44();campaignStateV44.actionsThisWeek=0;const boost=campaignStateV44.offices*75;campaignStateV44.volunteers+=boost;campaignStateV44.log.unshift(`Regional offices recruited ${boost} extra volunteers this week.`);};

/* v4.6 stable dashboard/campaign fix */
window.campaignV46 = window.campaignV46 || {
  volunteers:0, offices:1, adverts:0, actions:0, max:3,
  regions:[], seats:[], log:[]
};

function v46Money(v){v=Number(v||0);if(v>=1000000)return"£"+(v/1000000).toFixed(1)+"m";if(v>=1000)return"£"+Math.round(v/1000)+"k";return"£"+v}
function v46Poll(){try{if(fixedHq?.pollingHistory?.length)return Number(fixedHq.pollingHistory[fixedHq.pollingHistory.length-1])}catch(e){};try{if(generatedPolling?.party)return Number(generatedPolling.party)}catch(e){};return 5.2}
function v46Funds(){try{if(typeof fixedHq.funds==="number")return fixedHq.funds}catch(e){};return 2800000}
function v46Members(){try{if(generatedMembership?.total)return generatedMembership.total}catch(e){};return 14310}

function ensureV46(){
  const s=window.campaignV46;
  if(s.regions.length)return;
  const base=v46Poll();
  s.volunteers=Math.max(900,Math.round(v46Members()*0.18));
  s.regions=[
  ...
  s.log=["Campaign HQ opened.","Regional strategy prepared.","Target-seat list created."];
}

function showDashboardHome(){
  const h=document.getElementById("fixedDashboardHome"), d=document.getElementById("fixedDashboardDetail"), c=document.getElementById("hqDetailContent");
  if(h){h.style.display="grid";h.classList.add("v46-show");h.classList.remove("v46-hide","force-hidden","campaign-direct-hidden","stable-hide")}
  if(d){d.style.display="none";d.classList.add("v46-hide");d.classList.remove("v46-show","force-visible","campaign-direct-active","stable-show")}
  if(c){c.className="fixed-detail-grid";c.innerHTML=""}
  document.querySelectorAll(".fixed-hq-nav").forEach(b=>b.classList.toggle("active",b.textContent.toLowerCase().includes("dashboard")));
  try{renderFixedHq()}catch(e){}
}

function openCampaignStable(){
  ensureV46();
  const h=document.getElementById("fixedDashboardHome"), d=document.getElementById("fixedDashboardDetail"), t=document.getElementById("hqDetailTitle"), c=document.getElementById("hqDetailContent");
  if(!h||!d||!t||!c){alert("Campaign HQ could not load.");return}
  h.style.display="none";h.classList.add("v46-hide");h.classList.remove("v46-show","force-visible");
  d.style.display="block";d.classList.add("v46-show");d.classList.remove("v46-hide","force-hidden");
  t.textContent="Campaign HQ";c.className="v46-campaign-content";c.innerHTML=renderV46Campaign();
  document.querySelectorAll(".fixed-hq-nav").forEach(b=>b.classList.toggle("active",b.textContent.toLowerCase().includes("campaign")));
}

function renderV46Campaign(){
  const s=window.campaignV46,left=s.max-s.actions,targets=s.seats.filter(x=>x.target).length;
  const ready=Math.min(100,Math.round(18+targets*8+s.offices*7+s.adverts*6+left*2));
  const actions=[["Hold Rally","Cost: £75k. Boosts members, volunteers and polling.","rally"],["Launch Advert","Cost: £250k. Big visibility boost but expensive.","advert"],["Volunteer Drive","Cost: £40k. Builds activist strength.","volunteers"],["Target Seat Push","Cost: £95k. Improves selected target seats.","target"],["Regional Tour","Cost: £120k. Boosts regional support.","tour"]];
  return `<div class="v46-campaign-grid">
  <div class="v46-card"><h3>Campaign Overview</h3><div class="v46-stats"><div><span>Polling</span><strong>${v46Poll().toFixed(1)}%</strong></div><div><span>Funds</span><strong>${v46Money(v46Funds())}</strong></div><div><span>Volunteers</span><strong>${s.volunteers.toLocaleString("en-GB")}</strong></div><div><span>Actions Left</span><strong>${left}</strong></div></div></div>
  <div class="v46-card"><h3>Campaign Actions</h3><div class="v46-list">${actions.map(a=>`<div class="v46-row"><div><strong>${a[0]}</strong><p>${a[1]}</p></div><button onclick="runV46Action('${a[2]}')" ${left<=0?"disabled":""}>Run</button></div>`).join("")}</div></div>
  <div class="v46-card"><h3>Election Readiness</h3><div class="v46-stats"><div><span>Target Seats</span><strong>${targets}</strong></div><div><span>Offices</span><strong>${s.offices}</strong></div><div><span>Adverts</span><strong>${s.adverts}</strong></div><div><span>Readiness</span><strong>${ready}/100</strong></div></div></div>
  <div class="v46-card"><h3>Regional Support</h3><div class="v46-list">${s.regions.map((r,i)=>`<div class="v46-row"><div><strong>${r.name}</strong><small>${r.office?"Office active":"No office"} • Support ${r.support.toFixed(1)}%</small><div class="v46-meter"><div style="--w:${Math.min(100,r.support*5)}%;"></div></div></div><button onclick="openV46Office(${i})" ${r.office?"disabled":""}>${r.office?"Open":"Open Office"}</button></div>`).join("")}</div></div>
  <div class="v46-card"><h3>Target Seats</h3><div class="v46-list">${s.seats.map((seat,i)=>`<div class="v46-row"><div><strong>${seat.name}</strong><small>Held by ${seat.party} • Win chance ${seat.chance}% ${seat.target?"• Targeted":""}</small><div class="v46-meter"><div style="--w:${Math.min(100,seat.chance*2)}%;"></div></div></div><button onclick="toggleV46Seat(${i})">${seat.target?"Remove":"Target"}</button></div>`).join("")}</div></div>
  <div class="v46-card"><h3>Campaign Log</h3><div class="v46-list">${s.log.slice(0,8).map(l=>`<div class="v46-row"><div><strong>${l}</strong><small>Campaign record</small></div></div>`).join("")}</div></div></div>`
}

function v46Spend(cost){const s=window.campaignV46;if(s.actions>=s.max){alert("You have used all campaign actions this week. End Week to continue.");return false}if(v46Funds()<cost){alert("You do not have enough funds.");return false}try{fixedHq.funds-=cost}catch(e){}s.actions++;return true}
function v46Boost(a){try{const i=fixedHq.pollingHistory.length-1;fixedHq.pollingHistory[i]=+(fixedHq.pollingHistory[i]+a).toFixed(1)}catch(e){}}
function v46News(title,text){try{if(fixedHq?.news&&typeof createFixedNews==="function")fixedHq.news.unshift(createFixedNews("Campaign HQ",title,text,"positive",[]))}catch(e){}}

function runV46Action(type){
  ensureV46();const s=window.campaignV46;
  if(type==="rally"){if(!v46Spend(75000))return;v46Boost(.2);s.volunteers+=180;try{generatedMembership.total+=450}catch(e){}s.log.unshift("Held a launch rally.");v46News("Rally energises supporters","The campaign rally boosts membership, volunteers and visibility.")}
  if(type==="advert"){if(!v46Spend(250000))return;v46Boost(.35);s.adverts++;s.log.unshift("Launched a national advert campaign.");v46News("Advertising campaign goes live","Your adverts increase visibility but reduce campaign funds.")}
  if(type==="volunteers"){if(!v46Spend(40000))return;s.volunteers+=650;try{generatedMembership.total+=280}catch(e){}s.log.unshift("Ran a volunteer recruitment drive.");v46News("Volunteer drive succeeds","New activists join the campaign operation.")}
  if(type==="target"){if(!v46Spend(95000))return;s.seats.forEach(x=>{if(x.target)x.chance=Math.min(45,x.chance+4)});v46Boost(.08);s.log.unshift("Expanded target-seat campaign.");v46News("Target-seat operation expands","Your selected target seats become more competitive.")}
  if(type==="tour"){if(!v46Spend(120000))return;s.regions.forEach(r=>r.support=+(r.support+(r.office?.6:.25)).toFixed(1));v46Boost(.18);s.log.unshift("Completed a regional tour.");v46News("Regional tour completed","The leader's regional tour improves campaign reach.")}
  try{renderFixedHq()}catch(e){};openCampaignStable();
}
function toggleV46Seat(i){ensureV46();const seat=campaignV46.seats[i];if(!seat)return;seat.target=!seat.target;campaignV46.log.unshift((seat.target?"Targeted ":"Removed ")+seat.name+".");openCampaignStable()}
function openV46Office(i){ensureV46();const r=campaignV46.regions[i];if(!r||r.office)return;if(!v46Spend(180000))return;r.office=true;r.support=+(r.support+1.2).toFixed(1);campaignV46.offices++;campaignV46.log.unshift("Opened campaign office in "+r.name+".");v46News("New office opens in "+r.name,"The office improves regional support and local organising.");try{renderFixedHq()}catch(e){};openCampaignStable()}

const oldOpenV46=typeof openHqSection==="function"?openHqSection:null;
openHqSection=function(section){
  if(section==="dashboard")return showDashboardHome();
  if(section==="campaign")return openCampaignStable();
  const h=document.getElementById("fixedDashboardHome"),d=document.getElementById("fixedDashboardDetail"),t=document.getElementById("hqDetailTitle"),c=document.getElementById("hqDetailContent");
  if(!h||!d||!t||!c)return;
  h.style.display="none";d.style.display="block";t.textContent=({media:"Media Centre",policy:"Policy Unit",candidates:"Candidate Recruitment",party:"Party Management",finance:"Finance Office",research:"Research Department",map:"UK Campaign Map",polling:"Polling Centre",calendar:"Calendar"})[section]||"Section";
  c.className="fixed-detail-grid";c.innerHTML=`<div class="fixed-detail-card"><h3>${t.textContent}</h3><p>This section is ready to expand next. Campaign HQ and Dashboard are now stable.</p><button onclick="showDashboardHome()">Back to Dashboard</button></div>`;
}
const oldEndV46=typeof endWeek==="function"?endWeek:null;
endWeek=function(){if(oldEndV46)oldEndV46();ensureV46();campaignV46.actions=0;const boost=campaignV46.offices*75;campaignV46.volunteers+=boost;campaignV46.log.unshift("Regional offices recruited "+boost+" extra volunteers this week.")}
