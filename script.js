...
/* v4.9 Seat Projection System */
window.seatProjectionData = window.seatProjectionData || {};

function calculateSeatProjectionV49(){
  const poll=(window.generatedPolling?.party)||5;
  const seats=Math.max(0,Math.round((poll-4)*2.5));
  const firstMp=Math.max(0,Math.min(100,Math.round((poll-3)*12)));
  const targets=[
    {name:"Walsall South",chance:Math.min(75,Math.round(poll*4.5))},
    {name:"Birmingham Edgbaston",chance:Math.min(70,Math.round(poll*3.8))},
    {name:"Dudley North",chance:Math.min(65,Math.round(poll*3.5))}
  ];
  return {
...
const oldRenderFixedHqV49 = typeof renderFixedHq==="function" ? renderFixedHq : null;

if(oldRenderFixedHqV49){
  renderFixedHq = function(){
    oldRenderFixedHqV49();

    const projection=calculateSeatProjectionV49();
    const dashboard=document.getElementById("fixedDashboardHome");
    if(!dashboard || document.getElementById("seatProjectionCardV49")) return;

    const div=document.createElement("div");
    div.id="seatProjectionCardV49";
    div.className="seat-projection-card";
    div.innerHTML=`
      <h3>Projected Result</h3>
      <div class="seat-projection-grid">
        <div class="seat-projection-item"><strong>${projection.projectedSeats}</strong><br>Projected MPs</div>
        <div class="seat-projection-item"><strong>${projection.firstMp}%</strong><br>Chance of First MP</div>
        <div class="seat-projection-item"><strong>${projection.bestSeat.name}</strong><br>Best Target Seat</div>
        <div class="seat-projection-item"><strong>${projection.bestSeat.chance}%</strong><br>Win Chance</div>
      </div>
    `;
...
/* v5.0 Weekly loop fix: visible week, tasks and events */
function ensureWeeklyTasksV50() {
  if (!window.fixedHq) return;

  const week = fixedHq.week || 1;

  if (!Array.isArray(fixedHq.tasks) || fixedHq.tasks.length === 0) {
    fixedHq.tasks = [
      { id: "task-rally-" + week, title: "Hold a regional rally", impact: "High Impact", done: false },
      { id: "task-media-" + week, title: "Respond to the week's biggest story", impact: "Medium Impact", done: false },
      { id: "task-candidates-" + week, title: "Recruit local candidates", impact: "High Impact", done: false },
      { id: "task-funds-" + week, title: "Raise £100k in donations", impact: "Medium Impact", done: false }
    ];
  }
}

function ensureUpcomingEventsV50() {
  if (!window.hqUpcomingEvents || !Array.isArray(hqUpcomingEvents) || hqUpcomingEvents.length === 0) {
    const week = window.fixedHq?.week || 1;
    window.hqUpcomingEvents = [
      { title: "BBC Politics Live Interview", time: "In 3 days", type: "Media Opportunity" },
      { title: "Regional Campaign Rally", time: "In 1 week", type: "Campaign Event" },
      { title: "Policy Announcement", time: "In 2 weeks", type: "Major Event" },
      { title: "Local Election Test", time: "In " + Math.max(2, 6 - (week % 4)) + " weeks", type: "Election Test" }
    ];
  }
}

function renderUpcomingEventsV50() {
  ensureUpcomingEventsV50();

  const box = document.getElementById("hqUpcomingEvents");
  if (!box) return;

  box.innerHTML = hqUpcomingEvents.map((event, index) => `
    <button class="fixed-event-row" onclick="openEventDetailsV50(${index})">
      <span>${event.title}<br><small>${event.type}</small></span>
      <strong>${event.time}</strong>
    </button>
  `).join("");
...
function openEventDetailsV50(index) {
  ensureUpcomingEventsV50();
  const event = hqUpcomingEvents[index];
  if (!event) return;

  if (typeof openHqSection === "function") {
    openHqSection("calendar");
  }
...
function forceDashboardRefreshV50() {
  ensureWeeklyTasksV50();
  ensureUpcomingEventsV50();

  if (typeof renderFixedHq === "function") {
    renderFixedHq();
  }

  const weekEl = document.getElementById("hqWeek");
  if (weekEl && window.fixedHq) {
    weekEl.textContent = fixedHq.week || 1;
  }

  if (typeof renderHqTasks === "function") {
    renderHqTasks();
  }

  renderUpcomingEventsV50();

  if (typeof renderFixedPollingLine === "function") renderFixedPollingLine();
  if (typeof renderFixedNews === "function") renderFixedNews();
  if (typeof renderFixedPollingBreakdown === "function") renderFixedPollingBreakdown();
  if (typeof renderFixedEnvironment === "function") renderFixedEnvironment();
...
/* Override weekly result calculation so the visible week always moves forward */
const previousCalculateWeeklyResultV50 = typeof calculateWeeklyResultV48 === "function" ? calculateWeeklyResultV48 : null;
calculateWeeklyResultV48 = function() {
  if (!window.fixedHq) {
    window.fixedHq = {
      week: 1,
      pollingHistory: [5.2],
  ...
  const oldWeek = fixedHq.week || 1;
  const oldPoll = getWeekPollV48 ? getWeekPollV48() : (fixedHq.pollingHistory?.[fixedHq.pollingHistory.length - 1] || 5.2);
  const oldMembers = getWeekMembersV48 ? getWeekMembersV48() : (window.generatedMembership?.total || 14310);
  const oldFunds = getWeekFundsV48 ? getWeekFundsV48() : (fixedHq.funds || 2800000);

  const completedTasks = Array.isArray(fixedHq.tasks) ? fixedHq.tasks.filter(t => t.done).length : 0;
  const campaignActions = window.cleanCampaignState ? cleanCampaignState.actionsThisWeek : 0;
  const campaignOffices = window.cleanCampaignState ? cleanCampaignState.offices : 1;
  const adverts = window.cleanCampaignState ? cleanCampaignState.adverts : 0;

  const event = typeof generateWeeklyEventV48 === "function"
    ? generateWeeklyEventV48()
    : { title: "Campaign Momentum", text: "Your party continues building national attention.", poll: 0.1, members: 200, funds: 50000 };

  let pollChange = 0.05 + completedTasks * 0.08 + campaignActions * 0.09 + campaignOffices * 0.025 + adverts * 0.015 + (event.poll || 0);
  pollChange += (Math.random() * 0.16) - 0.06;

  let memberChange = 180 + completedTasks * 120 + campaignActions * 180 + campaignOffices * 75 + (event.members || 0) + Math.round(Math.random() * 220);
  let fundChange = 90000 + Math.round(oldMembers * 1.2) + (event.funds || 0) + Math.round(Math.random() * 65000);

  const newPoll = Math.max(0.5, Math.min(45, oldPoll + pollChange));
  const newMembers = Math.max(1000, oldMembers + memberChange);
  const newFunds = Math.max(0, oldFunds + fundChange);
  const newWeek = oldWeek + 1;

  fixedHq.week = newWeek;

  if (!Array.isArray(fixedHq.pollingHistory)) fixedHq.pollingHistory = [];
  fixedHq.pollingHistory.push(Number(newPoll.toFixed(1)));

  if (!window.generatedMembership) window.generatedMembership = { total: Math.round(newMembers) };
  generatedMembership.total = Math.round(newMembers);

  fixedHq.funds = Math.round(newFunds);

  fixedHq.tasks = [
    { id: "task-rally-" + newWeek, title: "Hold a regional rally", impact: "High Impact", done: false },
    { id: "task-media-" + newWeek, title: "Respond to the week's biggest story", impact: "Medium Impact", done: false },
    { id: "task-candidates-" + newWeek, title: "Recruit local candidates", impact: "High Impact", done: false },
    { id: "task-funds-" + newWeek, title: "Raise £100k in donations", impact: "Medium Impact", done: false }
  ];
  ...
  window.hqUpcomingEvents = [
    ...
    { title: "Regional Campaign Rally", time: "In 1 week", type: "Campaign Event" },
    { title: "Policy Announcement", time: "In 2 weeks", type: "Major Event" },
    { title: "Local Election Test", time: "In " + Math.max(2, 6 - (newWeek % 4)) + " weeks", type: "Election Test" }
  ];
  ...
  if (window.cleanCampaignState) {
    ...
    const volunteerBoost = cleanCampaignState.offices * 75;
    cleanCampaignState.volunteers += volunteerBoost;
    cleanCampaignState.log.unshift("Week " + newWeek + ": offices recruited " + volunteerBoost + " extra volunteers.");
  }

  if (!Array.isArray(fixedHq.news)) fixedHq.news = [];

  const stories = typeof generateWeeklyStoriesV48 === "function"
    ? generateWeeklyStoriesV48({
        oldWeek,
        newWeek,
        oldPoll,
        newPoll,
    ...
    : [
        { title: "Weekly polling update", text: "Your party moves to " + newPoll.toFixed(1) + "% nationally." },
        { title: event.title, text: event.text }
      ];

  stories.reverse().forEach(story => {
    if (typeof createFixedNews === "function") {
      fixedHq.news.unshift(createFixedNews("Week " + newWeek, story.title, story.text, "major", [
        { label: "Respond", effect: "credibility" },
        { label: "Use in Campaign", effect: "members" },
        { label: "Ignore", effect: "safe" }
      ]));
...
/* Override modal close so dashboard redraws properly */
closeWeeklySummaryV48 = function() {
  const modal = document.getElementById("weeklyModalV48");
  if (modal) modal.remove();
  forceDashboardRefreshV50();
};

/* Final End Week override */
endWeek = function() {
  const summary = calculateWeeklyResultV48();

  if (typeof showWeeklySummaryV48 === "function") {
    showWeeklySummaryV48(summary);
  } else {
    alert("Week " + summary.oldWeek + " complete. Now entering Week " + summary.newWeek + ".");
    forceDashboardRefreshV50();
  }
};
